...
When CFA-aware software encounters a file that contains a SAID placeholder, it knows that the filename MUST contain an actual SAID value, and it knows what that SAID value must be: *the hash of the file content, including the SAID placeholder*. It also knows that the filename MUST include the filename fragment `y`. If the file doesn't have such a name that matches this pattern, the filename is invalid.

This mechanism solves the chicken-and-egg problem of software not allowing a SAID value to be injected, because the SAID value is externalized to the filename that can be changed independent of (and after) the content. Yet the content and the filename still have a verifiable binding. Anybody who attempts to remove the SAID or the fragment from the filename OR change the content of the file leaves evidence of tampering. The filename derives from its content.

If a SAID placeholder is used as the `value` portion of a CFA `name=value` pair, and if the entropy portion of the SAID placeholder is an AID, then the CFA has a verifiable owner &mdash; whoever controls the keys of the AID. It also has a verifiable filename for its pre file, preventing attackers from manipulating the filename in a way that mischaracterizes it.

This still does NOT guarantee that the author of the file in its valid form is the original author; Malfoy could use an externalized SAID on a file that contains content copied from some other author. However, if the filename must contain the phrase "Alice's 9th Symphony", it's hard for Malfoy to appropriate the file himself. He has to rewrite the file in a way that removes (or injects a new) SAID placeholder &mdash; and presumably, that was hard even for the original author, which is why Alice used this type of CFA binding in the first place.

If Alice wants absolute proof that she is the author, she can combine this technique with a proof of existence at time X &mdash; e.g., writing the file to IPFS or a blockchain. Any party who produces substantially the same content but can only demonstrate a later timestamp has a derivative work.